...
describe("useNetworkStatus", () => {
  ...
  describe("polling stops when tab is hidden (async race)", () => {
    const simulateHide = () => {
      Object.defineProperty(document, "hidden", {
        writable: true,
        configurable: true,
    ...
    it("does not restart the polling loop after checkConnection completes while tab is hidden", async () => {
      vi.useFakeTimers();

      let resolveFetch!: (v: unknown) => void;
      const slowFetch = new Promise((res) => {
        resolveFetch = res;
      });

      const fetchMock = vi
        .fn()
        .mockResolvedValueOnce(OK_RESPONSE) // initial check
        .mockReturnValueOnce(slowFetch); // racing check (in-flight during hide)
      vi.stubGlobal("fetch", fetchMock);
      ...
      // Advance 30001ms — timer fires and second fetch starts (still in-flight)
      await act(async () => {
        vi.advanceTimersByTime(30_001);
      });

      // Simulate the async race: tab becomes hidden while checkConnection is in-flight
      simulateHide();
      fetchMock.mockClear();

      // Resolve the in-flight fetch — the guard should prevent scheduleNextCheck()
      resolveFetch(OK_RESPONSE);
      await act(async () => {});

      // Advance 60s — no new fetch should fire since the loop stopped
      await act(async () => {
        vi.advanceTimersByTime(60_000);
      });
      await act(async () => {});
    ...
    it("handleNetworkResume does not restart polling when tab is hidden", async () => {
      vi.useFakeTimers();
      ...
      // Fire mm-sse-reconnect → handleNetworkResume calls checkConnection
      // but must NOT call scheduleNextCheck() because document.hidden is true
      await act(async () => {
        window.dispatchEvent(new Event("mm-sse-reconnect"));
      });
      await act(async () => {});

      // Clear the checkConnection call made by handleNetworkResume itself
      fetchMock.mockClear();

      // Advance 60s — scheduleNextCheck was not called, so no new fetch
      await act(async () => {
        vi.advanceTimersByTime(60_000);
      });
      await act(async () => {});
  ...
  describe("browser online/offline events", () => {
    it("sets isOnline false immediately when window offline event fires (no fetch)", async () => {
      const fetchMock = vi.fn().mockResolvedValue(OK_RESPONSE);
      vi.stubGlobal("fetch", fetchMock);
      const nowSpy = vi.spyOn(performance, "now").mockReturnValue(0);
      ...
      expect(result.current.isOnline).toBe(false);
      expect(result.current.isSlow).toBe(false);
      // goOffline is synchronous — no fetch should be triggered
      expect(fetchMock).not.toHaveBeenCalled();

      nowSpy.mockRestore();
    });

    it("triggers checkConnection when window online event fires", async () => {
      vi.useFakeTimers();
      const fetchMock = vi.fn().mockResolvedValue(OK_RESPONSE);
      vi.stubGlobal("fetch", fetchMock);
      const nowSpy = vi.spyOn(performance, "now").mockReturnValue(0);
      ...
      renderHook(() => useNetworkStatus());
      await act(async () => {}); // initial check
      fetchMock.mockClear();

      // Dispatch browser online event — handleNetworkResume calls checkConnection
      await act(async () => {
