...
describe("useSmartSync", () => {
  ...
  describe("flush — success path", () => {
    ...
    it("keeps update op queued when ts changed mid-flush (superseded)", async () => {
      vi.mocked(getQueue).mockResolvedValue([makeOp()]);
      // snapshotTs captured at markInFlight time
      vi.mocked(markInFlight).mockResolvedValueOnce(1000);
      // User edited the op while batchUpdateAddress was in-flight → stored ts changed
  ...
  describe("follow-up flush for ops enqueued during active flush", () => {
    ...
    it("drains new ops immediately even when flush had transient failures", async () => {
      const a1 = makeOp({ addressId: "a1", opKey: "map-1:a1" });
      const a2 = makeOp({ addressId: "a2", opKey: "map-1:a2" });

      vi.mocked(getQueue)
        .mockResolvedValueOnce([a1]) // mount refreshCount
        .mockResolvedValueOnce([a1]) // flush snapshot
        .mockResolvedValueOnce([a2]) // enqueue's refreshCount (inside batchUpdateAddress)
        .mockResolvedValueOnce([a2]) // flush finally refreshCount
        .mockResolvedValueOnce([a2]) // drain flush snapshot
        .mockResolvedValue([]); // all subsequent

      let capturedEnqueue: ((op: QueuedOp) => Promise<void>) | undefined =
        undefined;
      vi.mocked(batchUpdateAddress)
        .mockImplementationOnce(async () => {
          // Enqueue a2 mid-flush — sets enqueuedDuringFlushRef / needsDrain
          if (capturedEnqueue) {
            await capturedEnqueue(a2);
          }
          throw new Error("network"); // transient failure for a1
        })
        .mockResolvedValueOnce(undefined); // a2 succeeds on immediate drain

      const { result } = renderHook(() => useSmartSync({ mapId: "map-1" }));
      capturedEnqueue = result.current.enqueue;
      await runAllTimers();

      // a1 failed transiently; a2 was drained immediately without waiting for backoff
      expect(batchUpdateAddress).toHaveBeenCalledTimes(2);
    });
  });
  ...
  describe("IN_FLIGHT state", () => {
    ...
    it("clears in-flight marker when a server write fails with a permanent error (404)", async () => {
      vi.mocked(getQueue).mockResolvedValue([makeOp()]);
      vi.mocked(batchUpdateAddress).mockRejectedValueOnce({ status: 404 });
      renderHook(() => useSmartSync({ mapId: "map-1" }));
      await runAllTimers();
  ...
  describe("Web Locks (navigator.locks)", () => {
    afterEach(() => {
      vi.unstubAllGlobals();
    });

    it("schedules a 2s retry when Web Lock is unavailable", async () => {
      const mockLocks = { request: vi.fn() };
      vi.stubGlobal("navigator", { ...navigator, locks: mockLocks });

      // First call: lock unavailable (cb receives null); second call: lock granted
      mockLocks.request
        .mockImplementationOnce(
          async (_key: unknown, _opts: unknown, cb: (lock: null) => void) =>
            cb(null)
        )
        .mockImplementation(
          async (
      ...
      renderHook(() => useSmartSync({ mapId: "map-1" }));
      // Let the first flush attempt settle (lock unavailable — no batchUpdateAddress)
      await act(async () => {});
      expect(batchUpdateAddress).not.toHaveBeenCalled();
      ...
      // The retry flush acquired the lock and processed the op
      expect(batchUpdateAddress).toHaveBeenCalledTimes(1);
    });

    it("does not schedule retry when component is unmounted at lock-skip time", async () => {
      const mockLocks = { request: vi.fn() };
      vi.stubGlobal("navigator", { ...navigator, locks: mockLocks });
      ...
      const { unmount } = renderHook(() => useSmartSync({ mapId: "map-1" }));
      // Unmount before or right as the 2s timer would be set — cleanup clears it
      unmount();
