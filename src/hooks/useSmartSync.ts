  ...
  const flushQueue = async () => {
    ...
    const runFlush = async () => {
      ...
      try {
        ...
        if (!unmountedRef.current) {
          ...
          // Always drain immediately if new ops arrived during this flush,
          // regardless of whether this flush had failures — the new ops haven't
          // been tried yet and should not wait for the failure backoff.
          if (needsDrain && isOnlineRef.current && !isSlowRef.current) {
            void flushQueueRef.current();
          }
        }
      }
    ...
    if ("locks" in navigator) {
      await navigator.locks.request(
        ...
        async (lock) => {
          if (!lock) {
            // Another tab is currently flushing. Schedule a short retry so
            // our ops don't wait for the next health-check tick.
            if (!flushTimerRef.current && !unmountedRef.current) {
              flushTimerRef.current = setTimeout(() => {
                flushTimerRef.current = null;
                if (isOnlineRef.current && !isSlowRef.current)
                  void flushQueueRef.current();
