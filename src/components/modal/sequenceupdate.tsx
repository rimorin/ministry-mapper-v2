...
interface SortableItemProps {
  ...
  code: string;
  sequence: number;
  mapType: "single" | "multi";
}

const SortableItem = ({ id, code, sequence, mapType }: SortableItemProps) => {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  const baseClasses = "sortable-item";
  const typeClasses =
    mapType === "single" ? "sortable-item-grid" : "sortable-item-horizontal";
...
const DragOverlayItem = ({
  ...
  const typeClasses =
    mapType === "single" ? "sortable-item-grid" : "sortable-item-horizontal";
  return (
    <div className={`sortable-item ${typeClasses} sortable-item-dragging`}>
      <span className="sortable-item-sequence">{sequence}</span>
      <span className="sortable-item-code">{code}</span>
    </div>
  );
...
const ChangeMapSequence = NiceModal.create(
  ({
    ...
    const [mapType, setMapType] = useState<"single" | "multi">("single");
    ...
    useEffect(() => {
      const fetchAddressList = async () => {
        ...
        console.log("Fetched codes:", response.codes);
        setCodeList(response.codes || []);
        setMapType(response.type || "single");
      };
      fetchAddressList();
    }, [mapId]);
