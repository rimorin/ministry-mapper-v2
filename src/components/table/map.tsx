  ...
  const organizeAddresses = (
    ...
    for (const address of addresses.values()) {
      ...
      if (!floorMap.has(floor)) {
        floorMap.set(floor, { floor, units: [] });
      }
      floorMap.get(floor)?.units.push(address);
    }

    // Convert floorMap to an array and sort units within each floor in-place
    const floorList = Array.from(floorMap.values());
    for (const floorDetails of floorList) {
      floorDetails.units.sort((a, b) => a.sequence - b.sequence);
    }

    // Sort floors by floor number in-place
    floorList.sort((a, b) => b.floor - a.floor);
